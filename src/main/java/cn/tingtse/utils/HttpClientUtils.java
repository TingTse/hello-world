...
public class HttpClientUtils {
     ...
     * 保存文件名
     */
    private static String saveFileName =System.getProperty("user.dir")+"/temp/"+String.valueOf(System.currentTimeMillis())+".pdf";
    /**
     * 返回消息
     */
     ...
     * @throws Exception
     */
    public static void getRespFile(HttpEntity entity,String saveFileName) throws Exception {
        if (entity == null) {
            System.out.println("获取的响应为空");
        }
        InputStream is = entity.getContent();
        // 定义一个文件名字进行接收获取文件
        FileOutputStream fileOut = new FileOutputStream(new File(saveFileName));
        ...
        if (fileOut.getFD().valid() == true) {
            message = "签名成功！文件保存至："+saveFileName;
